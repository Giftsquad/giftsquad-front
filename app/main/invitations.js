...
import { actionInvitations, getInvitations } from '../../services/eventService';
  ...
  const handleDeclineButton = async id => {
    ...
    try {
      ...
      if (response) {
        ...
        // Mettre à jour les événements dans le contexte global (au cas où)
        await refreshEvents();
  ...
  const handleAcceptButton = async id => {
    ...
    try {
      ...
      if (response) {
        ...
        // Mettre à jour les événements dans le contexte global
        await refreshEvents();
